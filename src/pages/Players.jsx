  ...
  return (
    <div className="p-4 space-y-4" dir="rtl">
      <div className="flex justify-between items-center">
        ...
        <button onClick={openAdd} className="px-4 py-2 rounded-xl bg-[#27c463] text-[#0b1220] hover:opacity-90">
          הוסף שחקן +
        </button>
      </div>
        ...
        <table className="w-full border-separate border-spacing-y-1">
          <thead className="sticky top-0 bg-[#0f1a2e]">
            <tr>
              ...
              <Th onClick={() => sortBy("must")} active={ui.sortBy==="must"}>חייב עם</Th>
              <Th onClick={() => sortBy("avoid")} active={ui.sortBy==="avoid"}>לא עם</Th>
              <Th>פעולות</Th>
            </tr>
          </thead>
          <tbody>
      ...
      <PlayerFormModal
        ...
        onSubmit={(pl) => {
          if (editPlayer) updatePlayer(pl); else addPlayer(pl);
          setModalOpen(false);
        }}
