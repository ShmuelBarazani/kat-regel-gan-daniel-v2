...
function teamStats(t) {
  ...
  const sum = t.players.reduce((s, p) => s + (p.rating || 0), 0);
  const avg = count ? sum / count : 0;
  return { count, sum, avg: Math.round(avg * 10) / 10 };
}
...
function violatesNotWith(team, player) {
  ...
  return player.notWith.some((n) => names.has(n));
}
    ...
    t.players.forEach((p) =>
      (p.mustWith || []).forEach((m) => {
        ...
        if (t2 !== undefined && t1 !== t2)
          warnings.add(`${p.name} חייב לשחק עם ${m} (כעת מופרדים)`);
      })
    )
  );
  return [...warnings];
}
function canDrop(team, player, max) {
  if (team.players.length >= max) return false;
...
export default function DoForces() {
  ...
  const targets = useMemo(
    () => sizeTargets(activeCount, teamsCount),
  ...
  function onClear() {
    setTeams(emptyTeams(teamsCount));
  }
  ...
  function overTeam(e, ti) {
    e.preventDefault();
    if (!drag) return;
    const t = teams[ti];
    const max = targets[ti];
    if (!canDrop(t, drag.player, max))
  ...
  function dropTeam(e, ti) {
    ...
    const t = teams[ti];
    const max = targets[ti];
    if (!canDrop(t, drag.player, max)) return endDrag();
    ...
    <div className="page" style={{ padding: "16px 12px" }}>
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <button className="btn" onClick={onMakeTeams}>
            עשה כוחות
          </button>
        ...
        {hoverWarn && (
          <div
            style={{
              ...
              padding: "8px 12px",
              borderRadius: 8,
              border: "1px dashed var(--edge,#334)",
        ...
        {useMemo(() => mustWithWarnings(teams), [teams]).length > 0 && (
          <div
            style={{
              ...
              padding: "8px 12px",
              borderRadius: 8,
              border: "1px solid var(--warn,#ff5c7a)",
              ...
              >
                ...
                >
                  ...
                  <div style={{ fontSize: 12, opacity: 0.85 }}>
                    ממוצע {stats.avg} | סכ״כ {stats.sum.toFixed(1)} |{" "}
                    {stats.count}/{target}
                  </div>
                </div>
      ...
      <section style={{ marginTop: 22 }}>
        ...
        >
          ...
          <small style={{ opacity: 0.8 }}>
            גרור שחקן מהטבלה לכרטיס קבוצה, או חזרה לכאן להסרה.
          </small>
        </div>

        <div
          onDragOver={overTable}
          onDrop={dropTable}
          style={{
            overflow: "auto",
            maxHeight: 420,
              ...
              {players.map((p) => {
                ...
                return (
                  <tr key={p.id}>
                    ...
                    <td style={tdSmall}>
                      {p.notWith?.length ? p.notWith.join(", ") : "—"}
                    </td>
                  </tr>
                );
              })}
...
const th = {
  textAlign: "right",
  padding: "10px 12px",
  borderBottom: "1px solid var(--edge,#24324a)",
  whiteSpace: "nowrap",
};
const td = {
  textAlign: "right",
