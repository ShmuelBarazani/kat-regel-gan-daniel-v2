...
// load/save players from localStorage (קיים אצלך)
...
export default function DoForces() {
  ...
  // צור חלוקה ראשונית אוטומטית (כמו בצילום—כרטיסים לא ריקים בטעינה)
  useEffect(() => {
    try {
      const t = generateTeams(players, teamCount);
      setTeams(t);
    } catch (e) {
      // אם לא אפשרי (למשל חייב-עם גדול מדי) נשאיר ריק עד לחיצה
      console.warn(e);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [teamCount]); // בכוונה לא על players כדי לא לדרוס עריכות ידניות
  ...
  const handleDropOnTeam = (e, idx) => { e.preventDefault(); dropToTeam(idx); dragRef.current = { playerId: null, fromTeamIdx: null }; };
  ...
  const toggleSort = (k) => (k === sortKey ? setSortDir((d) => (d === "asc" ? "desc" : "asc")) : (setSortKey(k), setSortDir("asc")));
  const togglePlaying = (id) => setPlayers((ps) => ps.map((p) => (p.id === id ? { ...p, playing: !p.playing } : p)));
  const onEditPos = (id, pos) => setPlayers((ps) => ps.map((p) => (p.id === id ? { ...p, pos } : p)));
  const onEditRating = (id, v) => { const num = clamp(parseFloat(v) || 0, 0, 10); setPlayers((ps) => ps.map((p) => (p.id === id ? { ...p, rating: num } : p))); };
  ...
  return (
    <div className="mx-auto px-3" dir="rtl">
      {/* bar: counters (left) & actions (right) */}
      <div className="flex items-center justify-between mt-4 mb-3">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setHideCardRatings((v) => !v)} className="rounded-xl border border-[#24324a] bg-[#0f1a2e] hover:bg-[#182544] px-3 py-2 text-sm">
            {hideCardRatings ? "הצג ציונים (בכרטיסים)" : "הסתר ציונים (בכרטיסים בלבד)"}
          </button>
          <button onClick={doMakeTeams} className="rounded-xl bg-green-600 hover:bg-green-700 px-3 py-2 text-sm text-white shadow">עשה כוחות</button>
          <button onClick={saveDraftCycle} className="rounded-xl bg-slate-600 hover:bg-slate-700 px-3 py-2 text-sm text-white">שמור מחזור (טיוטה)</button>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-[#24324a] bg-[#0f1a2e] mb-4">
        ...
        <div className="grid lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-3 p-3">
          {teams.map((team, idx) => (
            <div key={idx} className="rounded-xl border border-[#24324a] bg-[#0f1a2e]" onDragOver={handleDragOver} onDrop={(e) => handleDropOnTeam(e, idx)}>
              <div className="flex items-center justify-between px-3 py-2">
                <div className="text-slate-200 font-medium">קבוצה {idx + 1}</div>
                {/* לפי הדרישה: בלי מונה שחקנים */}
                <div className="text-[11px] text-slate-300">סכ״כ {teamStats[idx].sum} | ממוצע {teamStats[idx].avg}</div>
              </div>
              <div className="h-px bg-[#24324a]" />
              <div className="p-2">
                {team.length === 0 && <div className="text-[12px] text-slate-500 px-1 py-1">— אין שחקנים —</div>}
                {team.map((p) => (
                  <div key={p.id} draggable onDragStart={(e) => handleDragStart(e, p.id, idx)}
                    ...
                    {!hideCardRatings && <div className="text-[13px] text-slate-200">{p.rating}</div>}
                  </div>
                ))}
              </div>
            </div>
      ...
      {/* ===== טבלת השחקנים – למטה ===== */}
      <div className="rounded-2xl border border-[#24324a] bg-[#0f1a2e] overflow-hidden" onDragOver={handleDragOver} onDrop={handleDropBackToPool}>
        ...
        <div className="max-h-[460px] overflow-auto">
          <table className="w-full text-right">
            ...
            <tbody>
              {tablePlayers.map((p) => (
                <tr key={p.id} className="border-t border-[#13213a] hover:bg-[#101b31] cursor-grab" draggable onDragStart={(e) => handleDragStart(e, p.id, null)}>
                  ...
                  <td className="py-2 px-3 text-xs text-slate-300">{(p.mustWith || []).length ? p.mustWith.join(", ") : "—"}</td>
                  <td className="py-2 px-3 text-xs text-slate-300">{(p.avoidWith || []).length ? p.avoidWith.join(", ") : "—"}</td>
                  <td className="py-2 px-3">
                    <div className="flex items-center gap-2 text-xs">
                      <button onClick={() => onEditLists(p.id)} className="rounded-full bg-[#1e5a3f] hover:bg-[#23704e] text-white px-3 py-1">ערוך</button>
                      <select value={p.pos} onChange={(e) => onEditPos(p.id, e.target.value)} className="bg-[#0f1a2e] border border-[#24324a] rounded px-2 py-1">
                      ...
                      <input type="number" step="0.5" min={0} max={10} defaultValue={p.rating}
                        onBlur={(e) => onEditRating(p.id, e.target.value)} className="w-16 bg-[#0f1a2e] border border-[#24324a] rounded px-2 py-1" />
                    </div>
                  </td>
        ...
        <div className="text-xs text-slate-400 px-3 py-2">טיפ: גרור שחקן מהטבלה לאחת הקבוצות, או גרור מכרטיס קבוצה חזרה לטבלה.</div>
      </div>
