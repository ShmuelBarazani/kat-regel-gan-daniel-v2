...
export default function DoForces(){
  ...
  function makeTeams(){
    ...
    let i=0, f=true; // snake
    while(pool.length){
      const p = pool.shift();
      buckets[i].players.push(p);
      buckets[i].sum += Number(p.rating)||0;
      if (f){ if (i===n-1){ f=false; } else i++; }
      else  { if (i===0){ f=true; }  else i--; }
    }
    setTeams(buckets.map(b=>({...b, avg: b.players.length? (b.sum/b.players.length):0 })));
  }
          ...
          <table style={{width:"100%", borderCollapse:"separate", borderSpacing:0}}>
            <thead>
              <tr style={{position:"sticky", top:0, background:"#105340", color:"#e8eefc"}}>
                <th style={th}>שם</th><th style={th}>עמדה</th><th style={th}>ציון</th>
              </tr>
            </thead>
            <tbody>
              {active.map(p=>(
