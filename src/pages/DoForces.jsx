...
export default function DoForces(){
  ...
  const [teams, setTeams] = useState([]); // [{name, players:[{name,rating,pos}], sum, avg}]
  ...
  function makeTeams(){
    ...
    const buckets = Array.from({length:n}, (_,i)=>({ name:`קבוצה ${i+1}`, players:[], sum:0 }));
    // Snake distribution (מאזן)
    let forward = true, i=0;
    while(pool.length){
      const p = pool.shift();
      buckets[i].players.push(p);
      buckets[i].sum += Number(p.rating)||0;
      if (forward){ if (i===n-1){ forward=false; } else i++; }
      else { if (i===0){ forward=true; } else i--; }
    }
    const withAvg = buckets.map(b=>({ ...b, avg: b.players.length? (b.sum/b.players.length):0 }));
    setTeams(withAvg);
  }
  ...
  return (
    <div className="page" dir="rtl" style={{maxWidth:1180, margin:"24px auto", padding:"0 12px"}}>
      <div style={{display:"flex", gap:12, alignItems:"center", flexWrap:"wrap", marginBottom:10}}>
        <div className="stat">פעילים: <b>{active.length}</b></div>
        <div className="stat">לא פעילים: <b>{inactive.length}</b></div>
        <div className="stat">סה״כ: <b>{players.length}</b></div>
        <div style={{flex:1}} />
      ...
      <div className="card" style={{background:"#0f1a2e", border:"1px solid #24324a", borderRadius:16, padding:12, marginBottom:14}}>
        <div style={{maxHeight:"40vh", overflow:"auto"}}>
          <table style={{width:"100%", borderCollapse:"separate", borderSpacing:0}}>
            <thead>
              <tr style={{position:"sticky", top:0, background:"#105340"}}>
                <th style={th}>שם</th>
                <th style={th}>עמדה</th>
                <th style={th}>ציון</th>
              </tr>
      ...
      {teams.length>0 && (
        <>
          ...
          <div style={{display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))", gap:12}}>
            {teams.map((t,idx)=>(
              <div key={idx} className="teamCard" style={{background:"#0f1a2e", border:"1px solid #24324a", borderRadius:16, padding:12}}>
                <div style={{display:"flex", justifyContent:"space-between", marginBottom:6}}>
                  <b>{t.name}</b>
                  <span style={{color:"#9fb0cb"}}>{t.players.length} שחקנים | סכ״ר {t.sum.toFixed(1)} | ממוצע {t.avg.toFixed(2)}</span>
                </div>
      ...
      <style>{`
        ...
        .stat{background:#0f1a2e;border:1px solid #24324a;color:#e8eefc;border-radius:999px;padding:6px 10px}
...
const th = { padding:"10px 12px", textAlign:"right", color:"#e8eefc" };
