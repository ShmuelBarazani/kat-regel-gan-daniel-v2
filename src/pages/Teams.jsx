...
function canMovePlayer({ fromSize, toSize, totalPlaying, teamCount }) {
  ...
  // אחרי ההעברה – המקור קטן באחד, היעד גדל באחד
  const afterFrom = fromSize - 1;
  const afterTo = toSize + 1;
  return afterFrom >= minSize && afterFrom <= maxSize && afterTo >= minSize && afterTo <= maxSize;
}

export default function Teams({
  // מצפה לקבל מבחוץ (כבר קיים אצלך): players = [{id,name,pos,rating,playing:true},...]
  players = [],
  initialTeamsCount = 4,
}) {
  const [teamCount, setTeamCount] = useState(initialTeamsCount);
  const [teams, setTeams] = useState(() =>
    Array.from({ length: initialTeamsCount }, () => ({ name: "", players: [] }))
  );
  ...
  // יצירת מחזור (חלוקה מאוזנת קשיחה מידית)
  const makeRound = useCallback(() => {
    const sorted = [...playingPlayers].sort((a, b) => (b.rating ?? 0) - (a.rating ?? 0));
    const next = Array.from({ length: teamCount }, (_, i) => ({
      name: `קבוצה ${i + 1}`,
      players: [],
    }));
    // סנייק להבטחת איזון ציונים + ספירות
    const { minSize, maxSize } = calcMinMaxSizes(totalPlaying, teamCount);
    let dir = 1;
    let i = 0;
    for (const p of sorted) {
      // למצוא יעד חוקי (לא יחרוג מהמקסימום)
      let attempts = 0;
      while (attempts < teamCount && next[i].players.length >= maxSize) {
    ...
    // בדק איזון סופי: אף קבוצה לא מתחת ל־min ולא מעל max
    const ok = next.every(t => t.players.length >= minSize && t.players.length <= maxSize);
    if (!ok) {
      // תיקון קטן – להזיז עודפים/חוסרים עד התכנסות
      const over = [];
      const under = [];
      next.forEach((t, idx) => {
  ...
  // גרירה ידנית בין קבוצות – אוכף פער ≤1
  const movePlayer = useCallback(
    (player, fromIdx, toIdx) => {
      if (fromIdx === toIdx) return;
      const fromSize = teams[fromIdx].players.length;
      ...
      if (
        ...
        alert("לא ניתן להעביר – חייבים לשמור על איזון: פער גודל בין קבוצות עד ±1 בלבד.");
        return;
      }
  ...
  const removeFromTeam = useCallback(
    (player, fromIdx) => {
      // הסרה מותרת רק אם לא שוברת את המינימום
      const { minSize } = calcMinMaxSizes(totalPlaying, teamCount);
      if (teams[fromIdx].players.length - 1 < minSize) {
        alert("לא ניתן להסיר – תשבור את האיזון (מתחת למינימום המותר לקבוצה).");
        return;
      }
      const next = teams.map(t => ({ ...t, players: [...t.players] }));
      next[fromIdx].players = next[fromIdx].players.filter(p => p.id !== player.id);
